    ...
    l.fn.vectorMap = function(l) {
        ...
        if ("addMap" === l)
            ...
            if (("set" === l || "get" === l) && e[l][arguments[1]])
                return t[l + (arguments[1].charAt(0).toUpperCase() + arguments[1].substr(1))].apply(t, Array.prototype.slice.call(arguments, 2));
            (l = l || {}).container = this,
            t = new jvm.Map(l)
        }
        return this
...
function(l) {
    ...
    function t(l) {
        ...
        0 !== s || 0 !== i) {
            ...
            return n = Math.max(Math.abs(s), Math.abs(i)),
            (!d || n < d) && (p(t, d = n) && (d /= 40)),
            p(t, n) && (a /= 40,
            i /= 40,
            s /= 40),
    ...
    var c, d, l = ["wheel", "mousewheel", "DOMMouseScroll", "MozMousePixelScroll"], e = "onwheel"in document || 9 <= document.documentMode ? ["wheel"] : ["mousewheel", "DomMouseScroll", "MozMousePixelScroll"], u = Array.prototype.slice;
    if (o.event.fixHooks)
        for (var a = l.length; a; )
            o.event.fixHooks[l[--a]] = o.event.mouseHooks;
    var i = o.event.special.mousewheel = {
        version: "3.1.9",
        setup: function() {
            if (this.addEventListener)
                for (var l = e.length; l; )
                    this.addEventListener(e[--l], t, !1);
            else
                this.onmousewheel = t;
        ...
        teardown: function() {
            if (this.removeEventListener)
                for (var l = e.length; l; )
                    this.removeEventListener(e[--l], t, !1);
            else
...
jvm.$ = jQuery,
Array.prototype.indexOf || (Array.prototype.indexOf = function(l, t) {
    var e;
    if (null == this)
...
jvm.AbstractCanvasElement.prototype.addGroup = function(l) {
    ...
    return l ? l.node.appendChild(t.node) : this.node.appendChild(t.node),
    t.canvas = this,
    t
}
,
...
jvm.SVGCanvasElement.prototype.applyTransformParams = function(l, t, e) {
    ...
    this.transX = t,
    this.transY = e,
    this.rootElement.node.setAttribute("transform", "scale(" + l + ") translate(" + t + ", " + e + ")")
}
,
...
jvm.SVGShapeElement.prototype.applyAttr = function(l, t) {
    ...
    "fill" === l && jvm.isImageUrl(t) ? jvm.SVGShapeElement.images[t] ? this.applyAttr("fill", "url(#image" + jvm.SVGShapeElement.images[t] + ")") : jvm.whenImageLoaded(t).then(function(l) {
        (a = new jvm.SVGElement("image")).node.setAttributeNS("http://www.w3.org/1999/xlink", "href", t),
        a.applyAttr("x", "0"),
        a.applyAttr("y", "0"),
        a.applyAttr("width", l[0].width),
        a.applyAttr("height", l[0].height),
        (e = new jvm.SVGElement("pattern")).applyAttr("id", "image" + jvm.SVGShapeElement.imageCounter),
        e.applyAttr("x", 0),
        e.applyAttr("y", 0),
...
jvm.SVGImageElement.prototype.applyAttr = function(l, t) {
    ...
    this.width && this.applyAttr("x", t - this.width / 2)) : "cy" == l ? (this.cy = t,
...
jvm.VMLCanvasElement.prototype.setSize = function(l, t) {
    ...
    this.rootElement) {
        for (i = 0,
        s = (e = this.rootElement.node.getElementsByTagName("shape")).length; i < s; i++)
            e[i].coordsize = l + " " + t,
            e[i].style.width = l + "px",
            e[i].style.height = t + "px";
        for (i = 0,
        s = (a = this.node.getElementsByTagName("group")).length; i < s; i++)
            a[i].coordsize = l + " " + t,
...
jvm.VMLCanvasElement.prototype.applyTransformParams = function(l, t, e) {
    ...
    this.transX = t,
    this.transY = e,
    this.rootElement.node.coordorigin = this.width - t - this.width / 100 + "," + (this.height - e - this.height / 100),
...
jvm.VMLPathElement.pathSvgToVml = function(l) {
    ...
    return (l = l.replace(/(-?\d+)e(-?\d+)/g, "0")).replace(/([MmLlHhVvCcSs])\s*((?:-?\d*(?:\.\d+)?\s*,?\s*)+)/g, function(l, t, e) {
        ...
        case "V":
            ...
            "l" + r + "," + h;
        case "c":
            return s = r + e[e.length - 4],
            n = h + e[e.length - 3],
            r += e[e.length - 2],
            h += e[e.length - 1],
            "v" + e.join(",");
        case "C":
            return s = e[e.length - 4],
            n = e[e.length - 3],
            r = e[e.length - 2],
            h = e[e.length - 1],
            "c" + e.join(",");
        case "s":
            return e.unshift(h - n),
            e.unshift(r - s),
            s = r + e[e.length - 4],
            n = h + e[e.length - 3],
            r += e[e.length - 2],
...
jvm.VectorCanvas = function(l, t, e) {
    ...
    this.impl = "svg" == this.mode ? new jvm.SVGCanvasElement(l,t,e) : new jvm.VMLCanvasElement(l,t,e),
...
jvm.NumericScale.prototype = {
    setMin: function(l) {
        ...
        this.minValue = "function" == typeof this.normalize ? this.normalize(l) : l
    },
    setMax: function(l) {
        this.clearMaxValue = l,
        this.maxValue = "function" == typeof this.normalize ? this.normalize(l) : l
    },
    setScale: function(l) {
        var t;
        for (this.scale = [],
    ...
    getValue: function(l) {
        ...
        s = 0; s < this.scale.length - 1; s++)
            ...
            a.push(t),
            i += t;
        for (e = (this.maxValue - this.minValue) / i,
        s = 0; s < a.length; s++)
            a[s] *= e;
        for (s = 0,
        l -= this.minValue; 0 <= l - a[s]; )
            l -= a[s],
            s++;
    ...
    getTicks: function() {
        var l, t, e = [this.clearMinValue, this.clearMaxValue], a = e[1] - e[0], i = Math.pow(10, Math.floor(Math.log(a / 5) / Math.LN10)), s = 5 / a * i, n = [];
        for (s <= .15 ? i *= 10 : s <= .35 ? i *= 5 : s <= .75 && (i *= 2),
        e[0] = Math.floor(e[0] / i) * i,
        e[1] = Math.ceil(e[1] / i) * i,
        l = e[0]; l <= e[1]; )
            t = l == e[0] ? this.clearMinValue : l == e[1] ? this.clearMaxValue : l,
            n.push({
                label: l,
...
jvm.Legend.prototype.render = function() {
    var l, t, e, a, i = this.series.scale.getTicks(), s = jvm.$("<div/>").addClass("jvectormap-legend-inner");
    for (this.body.html(""),
    ...
    l = 0; l < i.length; l++) {
        ...
        t.append(jvm.$("<div>" + a + " </div>").addClass("jvectormap-legend-tick-text")),
        s.append(t)
    }
...
jvm.DataSeries = function(l, t, e) {
    ...
    this.scale = new a(l.scale,l.normalizeFunction,l.min,l.max)) : this.scale = l.scale ? new jvm.OrdinalScale(l.scale) : new jvm.SimpleScale(l.scale),
...
jvm.DataSeries.prototype = {
    setAttributes: function(l, t) {
        ...
        if ("string" == typeof l)
            ...
            for (e in a)
                this.elements[e] && this.elements[e].element.setStyle(this.params.attribute, a[e])
    },
    setValues: function(l) {
        var t, e, a = -Number.MAX_VALUE, i = Number.MAX_VALUE, s = {};
        if (this.scale instanceof jvm.OrdinalScale || this.scale instanceof jvm.SimpleScale)
            for (e in l)
                s[e] = l[e] ? this.scale.getValue(l[e]) : this.elements[e].element.style.initial[this.params.attribute];
        else {
            if (void 0 === this.params.min || void 0 === this.params.max)
                for (e in l)
                    a < (t = parseFloat(l[e])) && (a = t),
                    t < i && (i = t);
            for (e in void 0 === this.params.min ? (this.scale.setMin(i),
            this.params.min = i) : this.scale.setMin(this.params.min),
            void 0 === this.params.max ? (this.scale.setMax(a),
            this.params.max = a) : this.scale.setMax(this.params.max),
            l)
                "indexOf" != e && (t = parseFloat(l[e]),
                s[e] = isNaN(t) ? this.elements[e].element.style.initial[this.params.attribute] : this.scale.getValue(t))
        }
    ...
    clear: function() {
        ...
        for (l in this.values)
            this.elements[l] && (t[l] = this.elements[l].element.shape.style.initial[this.params.attribute]);
...
jvm.MapObject.prototype.getLabelOffsets = function(l) {
    var t;
    return this.config.label && ("function" == typeof this.config.label.offsets ? t = this.config.label.offsets(l) : "object" == typeof this.config.label.offsets && (t = this.config.label.offsets[l])),
    t || [0, 0]
}
,
...
jvm.Region = function(l) {
    ...
    this.config.label && e && (a = this.getLabelOffsets(l.code),
...
jvm.Marker = function(l) {
    ...
    this.config.label && t && (this.offsets = this.getLabelOffsets(l.index),
...
jvm.Marker.prototype.setStyle = function(l) {
    ...
    (t = !!this.shape.get("image")) != this.isImage && (this.isImage = t,
...
jvm.Map.prototype = {
    ...
    reset: function() {
        ...
        for (l in this.series)
            for (t = 0; t < this.series[l].length; t++)
    ...
    bindContainerEvents: function() {
        ...
        this.params.panOnDrag && (this.container.mousemove(function(l) {
            return a && (s.transX -= (t - l.pageX) / s.scale,
    ...
    bindContainerTouchEvents: function() {
        var n, r, h, o, m, p, c, d = this, l = function(l) {
            ...
            h = s[0].pageX,
            o = s[0].pageY) : 2 == s.length && (2 == c ? (e = Math.sqrt(Math.pow(s[0].pageX - s[1].pageX, 2) + Math.pow(s[0].pageY - s[1].pageY, 2)) / r,
            d.setScale(n * e, m, p),
            d.tip.hide(),
            l.preventDefault()) : (t = jvm.$(d.container).offset(),
            m = s[0].pageX > s[1].pageX ? s[1].pageX + (s[0].pageX - s[1].pageX) / 2 : s[0].pageX + (s[1].pageX - s[0].pageX) / 2,
            p = s[0].pageY > s[1].pageY ? s[1].pageY + (s[0].pageY - s[1].pageY) / 2 : s[0].pageY + (s[1].pageY - s[0].pageY) / 2,
            m -= t.left,
            p -= t.top,
            n = d.scale,
            r = Math.sqrt(Math.pow(s[0].pageX - s[1].pageX, 2) + Math.pow(s[0].pageY - s[1].pageY, 2)))),
            c = s.length
        };
    ...
    bindContainerPointerEvents: function() {
        ...
        (t.target = l).addEventListener("MSGestureChange", function(l) {
            var t, e;
            (0 != l.translationX || 0 != l.translationY) && (t = a.transX,
            e = a.transY,
            ...
            1 != l.scale && (a.setScale(a.scale * l.scale, l.offsetX, l.offsetY),
            a.tip.hide(),
            l.preventDefault())
        }, !1),
        l.addEventListener("pointerdown", function(l) {
    ...
    bindElementEvents: function() {
        ...
        this.container.delegate("[class~='jvectormap-element']", "mouseover mouseout", function(l) {
            ...
            "mouseover" == l.type ? (r.container.trigger(n, [e]),
            n.isDefaultPrevented() || a.setHovered(!0),
            r.tip.text(i),
            r.container.trigger(s, [r.tip, e]),
            s.isDefaultPrevented() || (r.tip.show(),
            r.tipWidth = r.tip.width(),
            r.tipHeight = r.tip.height())) : (a.setHovered(!1),
            r.tip.hide(),
        ...
        this.container.delegate("[class~='jvectormap-element']", "mouseup", function() {
            ...
            i || (r.container.trigger(e, [t]),
            ("region" === l && r.params.regionsSelectable || "marker" === l && r.params.markersSelectable) && (e.isDefaultPrevented() || (r.params[l + "sSelectableOne"] && r.clearSelected(l + "s"),
    ...
    setScale: function(l, t, e, a, i) {
        var s, n, r, h, o, m, p, c, d, u = jvm.$.Event("zoom.jvectormap"), v = this, M = 0, g = Math.abs(Math.round(60 * (l - this.scale) / Math.max(l, this.scale))), f = new jvm.$.Deferred;
        ...
        void 0 !== t && void 0 !== e && (zoomStep = l / this.scale,
        d = a ? (c = t + this.defaultWidth * (this.width / (this.defaultWidth * l)) / 2,
        e + this.defaultHeight * (this.height / (this.defaultHeight * l)) / 2) : (c = this.transX - (zoomStep - 1) / l * t,
    ...
    setFocus: function(l) {
        var t, e, a, i, s;
        if ((l = l || {}).region ? a = [l.region] : l.regions && (a = l.regions),
        a) {
            for (i = 0; i < a.length; i++)
                this.regions[a[i]] && ((e = this.regions[a[i]].element.shape.getBBox()) && (t = void 0 === t ? e : {
                    x: Math.min(t.x, e.x),
                    y: Math.min(t.y, e.y),
        ...
        return l.lat && l.lng ? (s = this.latLngToPoint(l.lat, l.lng),
        l.x = this.transX - s.x / this.scale,
        l.y = this.transY - s.y / this.scale) : l.x && l.y && (l.x *= -this.defaultWidth,
    ...
    getSelected: function(l) {
        var t, e = [];
        for (t in this[l])
            this[l][t].element.isSelected && e.push(t);
        return e
    },
    ...
    setSelected: function(l, t) {
        ...
        jvm.$.isArray(t))
            for (e = 0; e < t.length; e++)
                this[l][t[e]].element.setSelected(!0);
        else
            for (e in t)
                this[l][e].element.setSelected(!!t[e])
    },
    ...
    createRegions: function() {
        var l, t, e = this;
        for (l in this.regionLabelsGroup = this.regionLabelsGroup || this.canvas.addGroup(),
        this.mapData.paths)
            t = new jvm.Region({
                map: this,
    ...
    createMarkers: function(l) {
        ...
        jvm.$.isArray(l))
            ...
            t = 0; t < s.length; t++)
                l[t] = s[t];
        for (t in l)
            i = l[t]instanceof Array ? {
                latLng: l[t]
            } : l[t],
            !1 !== (a = this.getMarkerPosition(i)) && (e = new jvm.Marker({
                map: this,
    ...
    repositionMarkers: function() {
        ...
        for (l in this.markers)
            !1 !== (t = this.getMarkerPosition(this.markers[l].config)) && this.markers[l].element.setStyle({
                cx: t.x,
                cy: t.y
            })
    },
    ...
    removeMarkers: function(l) {
        var t;
        for (t = 0; t < l.length; t++)
            this.markers[l[t]].element.remove(),
            delete this.markers[l[t]]
    },
    ...
    latLngToPoint: function(l, t) {
        var e, a, i, s = jvm.Map.maps[this.params.map].projection, n = s.centralMeridian;
        return t < -180 + n && (t += 360),
        e = jvm.Proj[s.type](l, t, n),
        !!(a = this.getInsetForPoint(e.x, e.y)) && (i = a.bbox,
    ...
    pointToLatLng: function(l, t) {
        var e, a, i, s, n, r = jvm.Map.maps[this.params.map].projection, h = r.centralMeridian, o = jvm.Map.maps[this.params.map].insets;
    ...
    createSeries: function() {
        ...
        this.params.series)
            for (l = 0; l < this.params.series[t].length; l++)
                this.series[t][l] = new jvm.DataSeries(this.params.series[t][l],this[t],this)
    },
    remove: function() {
        this.tip.remove(),
...
jvm.MultiMap.prototype = {
    addMap: function(l, t) {
        ...
        return this.params.container.append(e),
        this.maps[l] = new jvm.Map(jvm.$.extend(t, {
            container: e
        })),
