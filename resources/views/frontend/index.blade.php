...
@section('content')
        ...
        <section class="welcome_area">
            <div class="welcome_slides owl-carousel">
                ...
                @foreach ($bannieres as $banniere)
                    <div class="single_slide bg-img" style="background-image: url({{$banniere->photo}});">
                        <div class="container h-100">
                            <div class="row h-100 align-items-center">
                                <div class="col-7 col-md-8">
                                    <div class="welcome_slide_text">
                                        <h2 style="color: white;" data-animation="fadeInUp" data-delay="300ms">{{$banniere->titre}}</h2>
                                        <h4 style="color: white;" data-animation="fadeInUp" data-delay="600ms">{!!html_entity_decode($banniere->description)!!}</h4>
                                        <a href="{{$banniere->slug}}" class="btn btn-primary" data-animation="fadeInUp" data-delay="1s">Découvrir</a>
                                    </div>
                                </div>
                ...
                @foreach ($categories as $cat)
                    <div class="col-12 col-md-4">
                        <div class="single_catagory_area mt-50">
                            <a href="{{route('categorie.produit',$cat->slug)}}">
                                <img src="{{$cat->photo}}" alt="{{$cat->titre}}" style="width: 400px; height: 350px;">
                            </a>
                        </div>
                    </div>
                @endforeach
    ...
    @php
        $nouveau_produits = \App\Models\Produit::where(['statut'=>'active', 'condition'=>'nouveau'])->orderBy('id','DESC')->limit(10)->get();
    @endphp

    @if(count($nouveau_produits) > 0)
        <!-- New Arrivals Area -->
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="new_arrivals_slides owl-carousel">
                            ...
                            @foreach ($nouveau_produits as $nproduit)
                                <div class="single-product-area">
                                    <div class="product_image">
                                        <!-- Product Image -->
                                        @php
                                            $photo = explode(',',$nproduit->photo);
                                        @endphp
                                        <img class="normal_img" src="{{$photo[0]}}" alt="" style="width: 300px; height: 250px;" alt="{{$nproduit->titre}}">
                                        <!--<img class="hover_img" src="" alt="" style="width: 300px; height: 250px;" alt="{{$nproduit->titre}}">-->

                                        <!-- Product Badge -->
                                        <div class="product_badge">
                                            <span>{{$nproduit->condition}}</span>
                                        </div>

                                        <!-- Wishlist -->
                                        <div class="product_wishlist">
                                            <a href="javascript:void(0);" class="add_to_wishlist" data-quantity="1" data-id={{$nproduit->id}} id="add_to_wishlist_{{$nproduit->id}}"><i class="icofont-heart"></i></a>
                                        </div>
                                    ...
                                    <div class="product_description">
                                        ...
                                        <div class="product_add_to_cart">
                                            <a href="#" data-quantity="1" data-product-id="{{$nproduit->id}}" class="add_to_cart" id="add_to_cart{{$nproduit->id}}"><i class="icofont-shopping-cart"></i> Ajouter au panier</a>
                                        </div>
                                        ...
                                        <p class="brand_name">{{\App\Models\Categorie::where('id',$nproduit->cat_id)->value('titre')}}</p>
                                        <a href="{{route('detail.produit',$nproduit->slug)}}">{{ucfirst($nproduit->titre)}}</a>
                                        @if ($nproduit->reduction != null)
                                            <h6 class="product-price">{{number_format($nproduit->offre_prix,2)}} CFA <small><del class="text-danger">{{number_format($nproduit->prix,2)}} CFA</del></small></h6>
                                        @else
                                            <h6 class="product-price">{{number_format($nproduit->offre_prix,2)}} CFA</h6>
                                        @endif
                                    </div>
                                </div>
                            @endforeach
